...
8. **TAG** — if the campaign adopts the [memo-tag spec](../specs/campaign-memo-tags.md): after the
   deposit confirms, emit the `E` boundary tag (`H1E:<pool>-<yymmdd>-<nnn>`, 1 µSTX transfer to
   the spec's tag sink, nonce serialized — INV-6/10) as a **Day-0 artifact**, same session as the entry. This mints the
